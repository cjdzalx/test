...
# 11。自动化的测试用例，每一条用例之间，不能有关联关系。每一条测试用例视为独立的测试用例。
...
if __name__ == '__main__':
	...
	dis = unittest.defaultTestLoader.discover(start_dir='', pattern="test_ME*.py")
	dia = unittest.defaultTestLoader.discover(start_dir='', pattern="test_OPT*.py")
	casetest.addTests(dis)
	casetest.addTests(dia)
